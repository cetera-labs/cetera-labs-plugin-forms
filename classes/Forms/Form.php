...
class Form extends \Cetera\DbObject {
    ...
    {
        foreach (\Forms\Form::enum() as $f) $data[] = $f->fields;
        return $data;
    }
     ...
     * @return string
     */
    public static function parseTemplate($template, $formID, $useTwig, $reCaptchaPublic, $errorMsg, $resultMsg, $twigParams)
    {
        self::$formID = $formID;
        ...
        $template = str_replace(array('[formerror]', '[formresult]'), array($errorMsg, $resultMsg), $template);
    ...
    private static function parseTagsRecursive($matches)
    {
        ...
        switch($matches[1])
        {
            ...
            case 'submit':
                ...
                else return '<input type="submit" name="send" value=' . $matches[3] . '/>'; // Совместимость с предыдущими версиями

            case 'submitbtn':
                $propsDelimiterPos = strpos($matches[3], '|');
                if($propsDelimiterPos !== false)
                    $fieldValue = substr($matches[3], 0, $propsDelimiterPos);
                else $fieldValue = $matches[3];
            ...
            case 'captchastrongimg':
                return '<img src="/plugins/forms/captcha.php?f=' . self::$formID . '"' . $fieldProps . '/>';
            ...
            case 'recaptcha':
                ...
                else return '<!-- Необходимо установить Recaptcha public key в настройках формы -->';
        ...
        if($useCaptcha !== false) // Проверка CAPTCHA
        {
            ...
            switch($useCaptcha) {
                ...
                case self::$CAPTCHA_TYPE_STRONG:
                    ...
                    else $resultStr = $captchaErrorText;
                    break;

                case self::$CAPTCHA_TYPE_RECAPTCHA:
                    $url = 'https://www.google.com/recaptcha/api/siteverify?secret=' . $form->reCaptchaPrivate . '&response='.(array_key_exists('g-recaptcha-response', $_POST) ? $_POST["g-recaptcha-response"] : '').'&remoteip='.$_SERVER['REMOTE_ADDR'];
    ...
    public static function performAction($form, $twigParams)
    {
		...
		{			
            ...
            $result = new \Forms\Result(array("text" => $data['mailBody']));
			...
			try {
				if (!empty($data['mailTo']) && $data['mailTo'] != '') { // отправка 1-ого письма
					...
					else self::sendMail($data, $form->mailHtml, $staticAttachments);
				}
				if($form->mail2 && !empty($data2['mailTo']) && $data2['mailTo'] != '') { // отправка 2-ого письма
					...
					else self::sendMail($data2, $form->mail2Html);
				}
    ...
    public static function getInputFileList($template) {
        ...
        foreach($arFileNames as $fileName) {
            if($_FILES[$fileName]['tmp_name'] != '')
                $arFiles[] = array('PATH' => $_FILES[$fileName]['tmp_name'], 'NAME' => $_FILES[$fileName]['name']);
        }
    ...
    public static function getTmplAttachments($sFiles) {
        ...
        foreach ($arPaths as $path) {
            ...
            if ($path != '' && file_exists(DOCROOT . $path))
                $arFiles[] = array('PATH' => DOCROOT . $path);
        }
    ...
    public static function parseTagsTemplateInValue($matches)
    {
        ...
        switch($matches[1])
        {
            ...
            default:
                ...
                else return $_REQUEST[$matches[1]];
        }
    }
